...
1. **[RESOLVED 2026-08-21]** ~~One real security-sensitive UI/DB mismatch, fail-safe but misleading.~~ The Edit Work modal's `canReassign` and the bulk-reassign toolbar's eligibility check are now admin-only, matching `guard_work_item_update()` exactly — a reviewer no longer sees a reassignment control that would silently fail on save. Fixed and covered by 4 new Playwright tests (see `docs/NEXT_STEPS_TRACKER.md` Group 2). This was the single most important finding in this map; it's closed now, not open.
2. **[STALE — corrected 2026-08-21]** ~~The "3 capability chapter" image consolidation has not started.~~ It's fully live: `capabilityChapter()` (`ui.js:109-134`) is wired into both Home (`pages1.js:199,215,231`) and Services (`pages2.js:24-33`), rendering the three real chapters — Establish & Comply / Run Your Finance Function / Advise & Report Better. `docs/DESIGN_SYSTEM.md` documents this correctly; this doc's own headline finding did not. The other 13 public pages never had a photo-per-category grid to begin with (they use a single hero photo + icon-based cards) — if a capability-chapter treatment is wanted there, that's a fresh design decision requiring new photography, not a continuation of started work.
...
5. **[RESOLVED 2026-08-21]** ~~One real CMS gap: the Resources page's content has zero admin-panel editor.~~ `admin.js` now has a full `sec-resources` editor (intro + all 4 tiles' title/text/CTA), matching every other page. Covered by 2 new admin-cms tests.
6. **[Reviewed 2026-08-21, mostly not a real issue]** ~~Several Work Desk sidebar labels don't match the page's own heading.~~ Checked current state: "Operations Overview" and "Catch-Up"/"Firm Work Catch-Up" are already consistent (both fixed since this was written). "Team Work"→"Team" and "My To-Do"→"My To-Do List" remain, but on reflection these are just normal shortened sidebar labels (same pattern as the fine "Catch-Up" one), not genuinely confusing — deliberately left as-is, not an oversight.
