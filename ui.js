...
function bulletList(items, cls = 'stamp-list') {
  return `<ul class="${cls}">${items.map(i => `<li>${stampMark('stamp-sm')}<span>${i}</span></li>`).join('')}</ul>`;
}

function serviceCard(cat) {
  return `<article class="service-card reveal" id="${cat.key}">
...
function processStep(p, isLast) {
  return `<div class="process-step reveal">
    <div class="process-step-marker"><span>${p.step}</span></div>
...
function ctaBand({ eyebrow: eb, title, subtitle, buttons }) {
  return `<section class="cta-band">
    <div class="container cta-band-inner reveal">
      ${eb ? `<p class="eyebrow eyebrow--on-dark">${eb}</p>` : ''}
      <h2>${title}</h2>
...
function trustBar(points) {
  return `<div class="trust-bar reveal">
    ${points.map(p => `<div class="trust-item">${icon('check', 'ic-trust')}<span>${p}</span></div>`).join('')}
  </div>`;
}
